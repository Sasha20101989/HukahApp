...
- Inventory: `/api/inventory`, `/api/inventory/{id}`, `/api/inventory/check`, `/api/inventory/in`, `/api/inventory/out`, `/api/inventory/adjustment`, `/api/inventory/movements`, `/api/inventory/dispatch-event`
...
Public endpoints are intentionally limited to auth, service catalog, health checks, read-only branch/mix/review/availability views, payment return status, YooKassa webhook and promocode validation. In production the YooKassa webhook must include `X-Tenant-Id` or signed tenant metadata and `X-Webhook-Secret`; Payment Service validates the secret against the active tenant-owned provider config.

Tenant payment provider management:

- `GET /api/payments/providers` lists provider configs for the current tenant.
