...
- Payments: `/api/payments/create`, `/api/payments/status/{id}`, `/api/payments/webhook/yookassa`, `/api/payments/{id}`, `/api/payments/{id}/refund`
...
Public endpoints are intentionally limited to auth, service catalog, health checks, read-only branch/mix/review/availability views, payment return status, YooKassa webhook and promocode validation. In production the YooKassa webhook must include `X-Webhook-Secret` matching `Payments:YooKassa:WebhookSecret`.
